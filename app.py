...
def make_pdf(df, mes_nome, ano):
    ...
    tf2 = tf
    ass = [
        [Paragraph("_______________________________", tf2),
         Paragraph("_______________________________", tf2)],
        [Paragraph("<b>Pároco: Pe. Pasquale Peluso</b>", tf2),
         Paragraph("<b>Secretária: Yolanda Facitela Clávio</b>", tf2)],
        [Paragraph("Data: ___/___/______", tf2),
         Paragraph("Data: ___/___/______", tf2)]
    ]
...
def make_excel(df, cols, mes_nome, ano):
    ...
    with pd.ExcelWriter(out, engine="openpyxl") as w:
        d = df[cols].copy() if cols and not df.empty else pd.DataFrame(columns=cols)
        if "Notas" in d.columns:
            d["Notas"] = d["Notas"].apply(clean)
...
if not df0.empty and "Mês" in df0.columns:
    dm0 = df0[
        (df0["Mês"].astype(str)==str(mes)) &
        (df0["Ano"].astype(str)==str(ano))
    ]
    if not dm0.empty:
        t0 = tot_min(dm0)
    ...
    if entrada and saida:
        ...
        if ok:
            ...
            with st.spinner("A guardar..."):
                if save(reg):
                    st.success(f"✅ Guardado! {data_obj.strftime('%d/%m/%Y')} — {h}")
                    st.balloons()
                    st.cache_data.clear()
        else:
            st.error(h)
